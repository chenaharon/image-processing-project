"""
Baseline Visualization Pipeline
...
def load_video(video_path: str, num_frames: int = 120) -> List[np.ndarray]:
    """Load video frames (grayscale, 64x64)."""
...
def extract_and_classify_blocks(frames: List[np.ndarray],
                                ...
                                confidence_threshold: float = CONFIDENCE_THRESHOLD) -> Tuple[List, List, List, List]:
...
def create_colored_frame(frame: np.ndarray, frame_idx: int,
                        ...
                        upscale: int = 8,  # 64*8 = 512 (as per prompt: 512x512)
                        id_to_label: Optional[Dict] = None,
                        invert_colors: bool = False,
                        activity_threshold: float = ACTIVITY_THRESHOLD,
                        confidence_threshold: float = CONFIDENCE_THRESHOLD) -> Tuple[np.ndarray, Dict]:
    """
    ...
    Args:
        ...
        upscale: Upscale factor (default: 32, so 64x64 -> 2048x2048)
    ...
    for block_info in block_predictions:
        ...
        # Activity filtering: skip if variance < threshold
        if variance < activity_threshold:
            stats['gray_blocks'] += 1
            color = COLORS['unclassified']
        # Confidence filtering: skip if confidence_ratio < threshold
            ...
            if id_to_label is not None and prediction in id_to_label:
                ...
                # Invert colors for hand_wave videos
                if invert_colors:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Visualize video with colored block overlays')
    ...
    parser.add_argument('--mapping', type=str, default=None, help='Path to label_mapping.pkl')
    ...
    mapping_path = Path(args.mapping) if args.mapping else (results_dir / 'label_mapping.pkl')
    ...
    frames = load_video(args.video, num_frames=120)
    ...
    all_positions, all_variances, active_positions, predictions, confidence_ratios = \
        extract_and_classify_blocks(frames, processor, selected_feature_indices, optimal_thresholds, stride, activity_threshold, confidence_threshold)
    ...
    for frame_idx in range(len(frames)):
        ...
        colored_frame, stats = create_colored_frame(
            ...
            activity_threshold=activity_threshold, confidence_threshold=confidence_threshold  # 64*8 = 512 (as per prompt)
        )
    ...
    # Print statistics
    total_blocks = sum(s['total_blocks'] for s in per_frame_stats)
    active_blocks = sum(s['active_blocks'] for s in per_frame_stats)
    classified_blocks = sum(s['classified_blocks'] for s in per_frame_stats)
    
    print(f"\nStatistics:")
    print(f"Activity filtering: {active_blocks}/{total_blocks} blocks active ({active_blocks/total_blocks*100:.1f}%)")
    print(f"Confidence filtering: {classified_blocks}/{active_blocks} blocks classified ({classified_blocks/active_blocks*100:.1f}%)")
